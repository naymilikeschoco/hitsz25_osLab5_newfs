...
int newfs_write(const char* path, const char* buf, size_t size, off_t offset,
	...
	int wsize;
	int blk_offset = offset % NEWFS_IO_SZ();
	int i = offset / NEWFS_IO_SZ();
	int wtotal = 0;
	while (i < NEWFS_DATA_PER_FILE && wtotal < size) {
		wsize = NEWFS_IO_SZ() - blk_offset;
		if (wtotal + wsize > size) {
			wsize = size - wtotal;
		}
		if (inode->data[i] == -1 || inode->data_blks[i] == NULL) {
			//分配数据块
			int dat_blk = newfs_alloc_data_blk();
			if (dat_blk < 0) {
				return wtotal;
			}
			inode->data[i] = dat_blk;
			inode->data_blks[i] = (uint8_t *)malloc(NEWFS_IO_SZ());
			memcpy(((uint8_t*)inode->data_blks[i]) + blk_offset, buf + wtotal, wsize);
		}
		i++;
		blk_offset = 0;
		wtotal += wsize;
	...
	// newfs_sync_inode(inode);
	
	//write根据写入数据长度进行磁盘数据块分配。
	return wtotal;
}
...
int newfs_read(const char* path, char* buf, size_t size, off_t offset,
	...
	if (offset + size > dentry->inode->size) {
		size = dentry->inode->size - offset; //调整读取大小，防止越界
	}
	//使用memcpy对内存中的文件内容进行读写。
	int readsize;
	int blk_offset = offset % NEWFS_IO_SZ();
	int i = offset / NEWFS_IO_SZ();
	int readtotal = 0;
	while (i < NEWFS_DATA_PER_FILE && readtotal < size) {
		if (inode->data[i] == -1) {
			break;
		}
		readsize = NEWFS_IO_SZ() - blk_offset;
		if (readtotal + readsize > size) {
			readsize = size - readtotal;
		}
		memcpy(buf + readtotal, ((uint8_t*)inode->data_blks[i]) + blk_offset, readsize);
		i++;
		blk_offset = 0;
		readtotal += readsize;
	}
...
int newfs_access(const char* path, int type) {
	...
	// struct newfs_dentry* dentry = 
	newfs_lookup(path, &is_find, &is_root);
	// struct newfs_inode*  inode;
